...
void init_dispmanx(int type, int width, int height)
{
	...
	ALOGD("init dispmanx type %d dims %dx%d\n", type, width, height);
...
void init_dispmanx(int type, int width, int height)
{
	...
	dev_fb = fopen("/dev/graphics/fb0", "r+b");
	if (!dev_fb)
	{
		ALOGE("could not open /dev/graphics/fb0\n");
		assert(0);
	}

	ALOGD("framebuffer opened ok\n");
...
//	printf("would update an image from %p type %d size %dx%d pitch %d\n", p, type, width, height, pitch);
	...
	if (!p)
	{
		ALOGE("image to update with is null\n");
		assert(0);
	}

	if (pitch > vinfo.xres * (vinfo.bits_per_pixel >> 3))
	{
		ALOGW("image pitch is larger than screen pitch! %dx%d, %d %d in\n", width, height, pitch, type);
		return;
	}

	if (height > vinfo.yres)
	{
		ALOGW("image height is larger than screen height %d\n", height);
		return;
	}
